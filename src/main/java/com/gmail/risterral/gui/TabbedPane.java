...
class TabbedPane extends JTabbedPane {
    ...
    private final TabbedPane instance;
    private final HashMap<String, Frame> popupFrames = new HashMap<>();
    ...
    public TabbedPane(final MainWindow mainWindow) {
        super();
        this.instance = this;
        final DragSourceListener dsl = new DragSourceListener() {
            ...
            public void dragDropEnd(DragSourceDropEvent e) {
                ...
                if (!mainWindow.getBounds().contains(e.getX(), e.getY())) {
                    final TabPanel selectedComponent = (TabPanel) instance.getSelectedComponent();

                    final WindowPopupDTO windowPopupDTO = ConfigurationController.getInstance().getConfigurationDTO().getWindowsPopups().containsKey(selectedComponent.getTabTitle()) ?
                            ConfigurationController.getInstance().getConfigurationDTO().getWindowsPopups().get(selectedComponent.getTabTitle()) : new WindowPopupDTO(e.getX(), e.getY());

                    final JFrame frame = new JFrame(selectedComponent.getTabTitle());
                    ...
                    frame.addWindowListener(new WindowAdapter() {
                        ...
                        public void windowClosed(WindowEvent e) {
                            ...
                            ConfigurationController.getInstance().getConfigurationDTO().getWindowsPopups().put(selectedComponent.getTabTitle(), windowPopupDTO);
                            ConfigurationController.getInstance().saveData();

                            GUIController.getInstance().addPanelToMainWindow(selectedComponent);
                            popupFrames.remove(selectedComponent.getTabTitle());

                        }
                    });
                    frame.getContentPane().add(selectedComponent);
                    frame.setVisible(true);
                    selectedComponent.update();

                    popupFrames.put(selectedComponent.getTabTitle(), frame);
    ...
    private int getTargetTabIndex(Point glassPt) {
        Point tabPt = SwingUtilities.convertPoint(
                glassPane, glassPt, TabbedPane.this);
    ...
    public HashMap<String, Frame> getPopupFrames() {
        return popupFrames;
    }
}